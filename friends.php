...
<?php
  ...
  $_base_utils->initialize_page_compass("Friends");
?>
<?php
        ...
        <div class="www-core-container">
            ...
            <div class="manage-top">
                <div style="width: 100%;border-top: 1px solid #CACACA;border-bottom: 1px solid #CACACA;">
                    <h3 style="margin-top: 0px;padding: 16px;"><span style="color: #03a;">My Account ▼ /</span> Friends</h3>
                </div>
            </div>
            <div class="manage-base">
                ...
                <table style="width: 100%;">
                    ...
                    <?php
                        ...
                        if($number_of_result == 0) {
                            echo '<span style="font-size: 11px;" class="grey-text">You have no outgoing friend requests.</span>';
                        }
                ...
                <?php for($page = 1; $page<= $number_of_page; $page++) { ?>
                    <a href="friends?page=<?php echo $page ?>">
                ...
                <table style="width: 100%;">
                    <?php
                        if($number_of_result == 0) {
                            echo '<span style="font-size: 11px;" class="grey-text">You have no incoming friend requests.</span>';
                        }
                    ?>
                    <tr>
                        <!-- <th style="margin: 5px; width: 5%;"></th> -->
                ...
                <?php } ?>   
                <?php
                    $stmt56 = $conn->prepare("SELECT * FROM friends WHERE reciever = ? AND (status != 'd' AND status != 'u')");
                    $stmt56->bind_param("s", $search);
                    $stmt56->execute();
                    $result854 = $stmt56->get_result();
                    $result56 = $result854->num_rows;
                    ...
                    $stmt = $conn->prepare("SELECT * FROM friends WHERE reciever = ? AND (status != 'd' AND status != 'u') ORDER BY id DESC");
                    $stmt->bind_param("s", $_SESSION['siteusername']);
                    $stmt->execute();
                    $result = $stmt->get_result();
                    $results = $result->num_rows;
                ...
                <table style="width: 100%;">
                    <?php
                        if($number_of_result == 0) {
                            echo '<span style="font-size: 11px;" class="grey-text">You have no friends. :(</span>';
                        }
                    ?>
                    <tr>
                        <!-- <th style="margin: 5px; width: 5%;"></th> -->
                    ...
                    <?php
                        $stmt6 = $conn->prepare("SELECT * FROM friends WHERE reciever = ? AND (status != 'd' AND status != 'u') ORDER BY id DESC LIMIT ?, ?");
                        $stmt6->bind_param("sss", $search, $page_first_result, $results_per_page);
                        $stmt6->execute();
                        $result6 = $stmt6->get_result();
                    ...
                    <tr style="margin-top: 5px;" id="videoslist">
                        ...
                        <td class="video-manager-stats">
                            <a href="/get/reject_friend_request?id=<?php echo $friend['id']; ?>">Unfriend</a><br>
                        </td>
                    </tr>
                    <?php } ?>
                </table> 
